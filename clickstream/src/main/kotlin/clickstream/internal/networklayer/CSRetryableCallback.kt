...
internal abstract class CSRetryableCallback(
    ...
    private suspend fun sendEvent() {
        if (sendOOMSafeEvent()) {
            logBatchSentEvent()
            logger.debug {
                "CSRetryableCallback#sendEvent - Request successfully sent to the server"
            }
        } else {
            logger.debug {
    ...
    private suspend fun sendOOMSafeEvent(): Boolean {
        return try {
            ...
            recordHealthEvent(
                ...
                eventType = CSEventTypesConstant.AGGREGATE,
                eventBatchId = eventRequest.reqGuid,
                error = "Out of memory while sending ${eventRequest.eventsList.size} events",
            )
            false
        }
    }
    ...
    private suspend fun observeTimeout() {
        ...
        timeOutJob = coroutineScope.launch(dispatcher) {
            ...
            recordHealthEvent(
                eventName = CSHealthEventName.ClickStreamEventBatchTimeout.value,
                eventType = CSEventTypesConstant.AGGREGATE,
                eventBatchId = eventRequest.reqGuid,
                error = "SocketTimeout"
            )
    ...
    private suspend fun sendAckAndComplete() {
        ...
        recordHealthEvent(
            eventName = CSHealthEventName.ClickStreamEventBatchAck.value,
            eventType = CSEventTypesConstant.AGGREGATE,
            eventBatchId = eventRequest.reqGuid,
            error = ""
        )
        ...
        when (eventResponse.code.ordinal) {
            Code.MAX_CONNECTION_LIMIT_REACHED.ordinal -> {
                ...
                recordHealthEvent(
                    ...
                    eventType = CSEventTypesConstant.AGGREGATE,
                    error = CSErrorConstant.MAX_CONNECTION_LIMIT_REACHED,
            ...
            Code.MAX_USER_LIMIT_REACHED.ordinal -> {
                ...
                recordHealthEvent(
                    eventName = CSHealthEventName.ClickStreamConnectionFailed.value,
                    eventType = CSEventTypesConstant.AGGREGATE,
                    error = CSErrorConstant.MAX_USER_LIMIT_REACHED,
                    eventBatchId = eventRequestGuid
                )
            }
            Code.BAD_REQUEST.ordinal -> {
                ...
                recordHealthEvent(
                    eventName = CSHealthEventName.ClickStreamEventBatchErrorResponse.value,
                    eventType = CSEventTypesConstant.AGGREGATE,
                    error = CSErrorConstant.PARSING_EXCEPTION,
                    eventBatchId = eventRequestGuid
                )
            }
            else -> {
    ...
    private suspend fun recordHealthEvent(
        ...
        logger.debug {
            StringBuilder()
                .append("CSRetryableCallback#recordHealthEvent")
                .apply {
        ...
        if (eventRequest.eventsCount > 0 && eventRequest.getEvents(0).isHealthEvent().not()) {
            ...
            healthProcessor?.insertBatchEvent(
                ...
                eventRequest.eventsCount.toLong()
            )
        }
    }
}