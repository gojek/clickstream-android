...
internal class CSHealthMetricsConnectionFailedTest {
    ...
    @Test
    fun `verify connection failure dto`() = runBlocking {
        ...
        val connFailureDto = CSHealthEventDTO(
        ...
        verify(healthEventRepository).insertHealthEvent(connFailureDto)
    ...
    private class FakeCSEventService : CSEventService {
        ...
        override fun observeSocketState(): Flow<WebSocket.Event> {
            return flow {
                emit(WebSocket.Event.OnConnectionFailed(SocketTimeoutException("broken")))
            }
        }
